...
<head>
    ...
    <style>
		...
		btn-check:checked+.btn, .btn.active, .btn.show, .btn:first-child:active, :not(.btn-check)+.btn:active{color: var(--primary); background-color: var(--primary-s); border-color: var(--primary-s);}
...
<body>
    <div class="container-fluid">
        <div class="row left-side">
            <div class="col-12 col-lg-5 vh-100 bg-white shadow-lg z-1 d-flex flex-wrap align-items-center align-content-around justify-content-center" id="left">
                ...
                <form class="login-box w-75" method="post" action="{{ route('member.profile.update') }}">
                    @csrf
                    <div class="my-4 px-2 scroll-box" style="height:calc(100vh - 25em); overflow-y:auto; overflow-x:hidden">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->name }}" autofocus>
                            @if($errors->has('name'))
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select {{ $errors->has('gender') ? 'border-danger' : '' }} rounded-3" name="gender">
                                ...
                                <option value="L" {{ Auth::user()->attribute && Auth::user()->attribute->gender == 'L' ? 'selected' : '' }}>Laki-Laki</option>
                                <option value="P" {{ Auth::user()->attribute && Auth::user()->attribute->gender == 'P' ? 'selected' : '' }}>Perempuan</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="birthdate" class="form-control {{ $errors->has('birthdate') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->attribute && Auth::user()->attribute->birthdate ? date('d/m/Y', strtotime(Auth::user()->attribute->birthdate)) : '' }}" placeholder="dd/mm/yyyy" autocomplete="off">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="phone_number" class="form-control {{ $errors->has('phone_number') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->attribute && Auth::user()->attribute->phone_number ? Auth::user()->attribute->phone_number : '' }}">
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="occupation" class="form-control {{ $errors->has('occupation') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->attribute && Auth::user()->attribute->occupation ? Auth::user()->attribute->occupation : '' }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="position" class="form-control {{ $errors->has('position') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->attribute && Auth::user()->attribute->position ? Auth::user()->attribute->position : '' }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="institution" class="form-control {{ $errors->has('institution') ? 'border-danger' : '' }} rounded-3" value="{{ Auth::user()->attribute && Auth::user()->attribute->institution ? Auth::user()->attribute->institution : '' }}">
                            @if($errors->has('institution'))
                            <div class="small text-danger text-start">{{ $errors->first('institution') }}</div>
                            @endif
                        </div>
                    </div>
                    <button class="w-100 btn btn-primary rounded-3 mb-3" type="submit">Update</button>
                    <a class="w-100 btn btn-secondary rounded-3 mb-3" href="{{ route('member.dashboard') }}">Kembali</a>
                </form>
                <div class="w-75"></div>
            </div>
    ...
    <script>
        ...
        Spandiv.Select2ServerSide("#select2", {
            ...
            value: "{{ Auth::user()->attribute && Auth::user()->attribute->country_code ? Auth::user()->attribute->country_code : '' }}",
            valueProp: "code",
            nameProp: "name",
